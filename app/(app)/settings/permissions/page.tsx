...
export default function PermissionsPage() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-lg font-semibold text-neutral-900">Hak Akses</h1>
        <p className="text-sm text-neutral-500">Atur permission untuk setiap role. Perubahan tersimpan otomatis.</p>
      </div>
              ...
              {PERMISSION_KEYS.map((key) => (
                <tr key={key}>
                  <td className="px-4 py-2.5">
                    <p className="font-medium text-neutral-800">{PERMISSION_LABEL[key]}</p>
                  ...
                  {(["admin", "sales"] as const).map((role) => (
                    <td key={role} className="px-4 py-2.5 text-center">
                      <input
                        ...
                        className="h-4 w-4 rounded border-neutral-300 text-forest-700 focus:ring-forest-600"
                      />
                    </td>
                  ))}
                </tr>
