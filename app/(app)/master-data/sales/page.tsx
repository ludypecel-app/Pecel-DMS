...
export default function SalesPage() {
  ...
  function openDeleteModal(sales: Sales) {
    setDeleteTarget(sales);
    setDeleteError(null);
  }
  ...
  async function confirmDelete() {
    ...
    setDeleting(true);
    setDeleteError(null);
    const res = await fetch(`/api/master-data/sales/${deleteTarget.id}?permanent=true`, { method: "DELETE" });
    const json = await res.json();
    setDeleting(false);
    if (!res.ok) {
    ...
    { key: "status", header: "Status", render: (s) => <StatusBadge status={s.status} /> },
    {
      ...
      render: (s) => (
        <div className="flex gap-3">
          ...
          <button type="button" onClick={() => handleToggleStatus(s)} className="text-sm font-medium text-ink-muted hover:underline">
            {s.status === "active" ? "Nonaktifkan" : "Aktifkan"}
          </button>
          <button type="button" onClick={() => openDeleteModal(s)} className="text-sm font-medium text-danger hover:underline">
            Hapus
          </button>
        </div>
      ),
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="h1 !text-[20px]">Sales</h1>
          <p className="text-sm text-ink-muted">Kelola data tenaga pemasaran</p>
        </div>
      ...
      <ConfirmModal
        open={!!deleteTarget}
        title="Hapus Sales"
        message={`Hapus permanen sales "${deleteTarget?.name}"? Tindakan ini tidak bisa dibatalkan.`}
